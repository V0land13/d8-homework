...
def index(request):
    ...
    cat_counts = Category.objects.all()
    counts = {cc.slug: cc.todos_count for cc in cat_counts}
    pr_counts = Priority.objects.all()
    pcounts = {pp.slug: pp.priority_count for pp in pr_counts}
    print(pcounts)
...
def date_page(request):
    ...
    return render(request, "tasks/date_page.html", {"now": now})